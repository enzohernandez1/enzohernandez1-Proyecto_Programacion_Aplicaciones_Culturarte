...
import logica.DTInfoAporte;
import logica.DTPropCompleto;
import logica.DTPropPersona;
...
//---------------------------------------------------------------------------------------------------------------------------------------------------
	 ...
	 private void mostrarDetalleProp(int fila) { //actualizar los JTextField
			...
			if (dto != null) {
				...
				if (dto.getTipoRetorno() == logica.TipoRetorno.AMBOS_RETORNOS) {
					combo1.addItem(logica.TipoRetorno.ENTRADA_GRATIS);
					combo1.addItem(logica.TipoRetorno.PORCENTAJE_GANANCIAS);
				} else {
