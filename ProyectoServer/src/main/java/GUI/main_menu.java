...
public class main_menu extends JFrame {
	...
	public main_menu() {
		...
		mnOtros.add(btnConsultarColaboracion);

		JButton btnEvaluarPropuesta = new JButton("Evaluar Propuesta");
		btnEvaluarPropuesta.setBackground(new Color(255, 255, 255));
		btnEvaluarPropuesta.setForeground(new Color(0, 0, 0));
		btnEvaluarPropuesta.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				EvaluarPropuestaView evaluarPropuesta = new EvaluarPropuestaView();
				desktopPane.add(evaluarPropuesta);
				evaluarPropuesta.setVisible(true);
				int x = (desktopPane.getWidth() - evaluarPropuesta.getWidth()) / 2;
				int y = (desktopPane.getHeight() - evaluarPropuesta.getHeight()) / 2;
				evaluarPropuesta.setLocation(Math.max(x,0), Math.max(y,0));
				evaluarPropuesta.setClosable(false);
				evaluarPropuesta.setMaximizable(false);
				evaluarPropuesta.setIconifiable(false);
				evaluarPropuesta.setResizable(false);
				javax.swing.plaf.basic.BasicInternalFrameUI ui = (javax.swing.plaf.basic.BasicInternalFrameUI) evaluarPropuesta.getUI();
				ui.setNorthPane(null);
			}
		});
		mnOtros.add(btnEvaluarPropuesta);
