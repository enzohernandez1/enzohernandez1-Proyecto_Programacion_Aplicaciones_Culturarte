...
public class AltaPerfilView extends JInternalFrame {
	...
	private boolean verificarCampos() {
		...
		if(txtUsuario.getText().trim().isEmpty() || txtNombre.getText().trim().isEmpty() || txtApellido.getText().trim().isEmpty() || txtCorreo.getText().trim().isEmpty()){
			return false;
		}
	...
	// setLocationRelativeTo no existe en JInternalFrame
		...
		spFecha.setBounds(228, 189, 83, 22);
		...
		cmbTipoUsuario.setBounds(218, 223, 114, 26);
		...
		lblImagen.setBounds(409, 86, 114, 106);
		...
		btnCargarImagen.setBounds(409, 204, 114, 27);
		pnlUsuario.add(btnCargarImagen);
		
		JLabel lblTipoDeUsuario = new JLabel("Tipo de usuario:");
		lblTipoDeUsuario.setBounds(103, 228, 107, 17);
	...
	taBiografia = new JTextArea();
        ...
        private boolean modificando = false;
			public void insertUpdate(DocumentEvent e) {
				...
				if(taBiografia.getText().length() >= 10000) {
					modificando = true;
		...
		lblFotoPerfil.setBounds(388, 59, 147, 17);
		...
		btnDarAlta.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(verificarFecha((Date)spFecha.getValue()) && verificarCampos()) {
		...
		btnDarAlta.setBounds(316, 500, 106, 27);
